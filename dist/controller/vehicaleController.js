...
const vehicaleController = {
    async getPublicVehicales(req, res) {
    ...
    async updateVehicale(req, res) {
        try {
            ...
            files = files.filter(file => file && (file.buffer !== undefined || file.path));
            let newMedia = [];
            ...
            if (req.body.existingImageUrls !== undefined && req.body.existingImageUrls !== null) {
                ...
                if (Array.isArray(req.body.existingImageUrls)) {
                    incomingUrls = req.body.existingImageUrls.map((m) => typeof m === 'string' ? m : m.url);
                }
