...
import HttpException, { BadRequestException, NotFoundException, UnauthorizedException, resolveStatus, } from '../services/HttpsException.js';
import { sendEmailVerifcationMail, sendNewAccountMail, sendPasswordResetMail, } from '../services/transport.js';
...
import { getErrorStatus, getErrorMessage } from '../types/errors.js';
...
authRouter.get('/', async (req, res) => {
    try {
        ...
        const user = (await Customer.findById(decodedToken._id).populate('role')) ||
            (await Dealer.findById(decodedToken._id).populate('role'));
        if (!user) {
        ...
        if (error instanceof TokenExpiredError) {
            await handleTokenExpiredError(req, res);
            return;
        }
        else {
...
const handleTokenExpiredError = async (req, res) => {
    const cookieOptions = getAuthCookieConfig({
        maxAge: 0,
        expires: new Date(0),
    });
    if (process.env.NODE_ENV !== 'production') {
        return res.clearCookie(cookieVariable, { ...cookieOptions, domain: 'localhost' });
    }
    res.clearCookie(cookieVariable, cookieOptions);
...
authRouter.post('/login', async (req, res) => {
    ...
    try {
        ...
        res.cookie(cookieVariable, token, getAuthCookieConfig());
        res.json({ ...user.toObject(), token });
    }
    catch (e) {
        res.status(getErrorStatus(e)).json({ message: getErrorMessage(e), errorCode: e instanceof HttpException ? e.errorCode : undefined });
    }
});
...
authRouter.get('/email-verification', async (req, res) => {
    try {
        ...
        const user = (await Customer.findByIdAndUpdate(decodedToken._id, { $set: { isEmailVerified: true } }, { new: true })) || (await Dealer.findById(decodedToken._id, { $set: { isEmailVerified: true } }, { new: true }));
        if (!user) {
...
authRouter.post('/google-login', async (req, res) => {
    try {
        ...
        let user = await Dealer.findOne({ email: payload?.email });
        if (!user) {
            user = await Dealer.create({
                email: payload?.email,
        ...
        const isProd = process.env.NODE_ENV === 'production';
        const cookieOptions = getAuthCookieConfig(isProd ? {} : { domain: 'localhost' });
        res.cookie(cookieVariable, token, cookieOptions);
        res.json({ ...user?.toObject() });
    }
    catch (e) {
        res.status(resolveStatus(e)).json({
...
authRouter.get('/logout', async (req, res) => {
    try {
        ...
        res.clearCookie(cookieVariable, cookieOptions);
        if (process.env.NODE_ENV !== 'production') {
            res.clearCookie(cookieVariable, {
                ...cookieOptions,
                domain: 'localhost',
            });
        }
...
authRouter.post('/forgot-password', async (req, res) => {
    try {
        ...
        res.status(resolveStatus(e)).json({ message: getErrorMessage(e) });
    }
});
...
authRouter.put('/change-password', async (req, res) => {
    try {
        ...
        const userModel = (await Customer.findById(decoded._id)) ||
...
authRouter.put("/update-user", (req, res, next) => {
    ...
    try {
        ...
        if (req.file) {
            ...
            const isCloudinaryUpload = req.file.path;
            if (isCloudinaryUpload) {
                // With Cloudinary storage, file is already uploaded and URL is available
                const imageUrl = req.file.path; // Cloudinary provides the URL in the path property
