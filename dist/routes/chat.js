...
chatRoute.get('/search', async (req, res) => {
    try {
        ...
        if (Dealer.schema.paths.username) { // Check if username field exists in schema
            dealerSearchFields.push({ username: searchRegex });
        }
        const dealerQuery = {
            $or: dealerSearchFields,
        };
        if (currentUserId) {
            dealerQuery._id = { $ne: currentUserId };
        }
        ...
        if (Customer.schema.paths.username) { // Check if username field exists in schema
            customerSearchFields.push({ username: searchRegex });
        }
        const customerQuery = {
        ...
        if (currentUserId) {
            customerQuery._id = { $ne: currentUserId };
        }
    ...
    try {
        ...
        res.status(201).json({
            ...
            data: {
                ...
                type: type,
                ...(type === 'd2d' && {
                    dealers: chatRoom.dealers
                }),
                ...(type === 'd2c' && {
                    dealer: chatRoom.dealer,
    ...
    try {
        ...
        if (type === 'd2d') {
            ...
            if (chat) {
                ...
                isAuthorized = chat.dealers.includes(userId);
                if (isAuthorized) {
            ...
            if (chat) {
                ...
                isAuthorized = chat.dealer.toString() === userId ||
