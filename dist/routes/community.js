...
}).test('text-or-media-required', 'Please provide at least text or media.', function (value) {
    const hasText = Boolean(value?.text?.trim());
    const hasMedia = Boolean(value?.media?.length);
...
communityRoute.post('/', upload.array('media'), async (req, res) => {
    try {
        ...
        const isCloudinaryUpload = files.length > 0 && files[0] && files[0].path;
        let media;
...
communityRoute.get('/', async (req, res) => {
    try {
        ...
        const search = (req.query.searchTerm || '').trim();
        const skip = (page - 1) * limit;
        // Search filter
        const searchFilter = search
            ? {
...
communityRoute.put('/:postId/like', async (req, res) => {
    ...
    try {
        ...
        const alreadyLiked = post.likes.some((like) => like.likedByUser?.toString() === userId);
        if (alreadyLiked) {
            post.likes = post.likes.filter((like) => like.likedByUser?.toString() !== userId);
        }
        else {
            post.likes.push({ likedByUser: userId });
        }
        await post.save();
        res.json({
            success: true,
