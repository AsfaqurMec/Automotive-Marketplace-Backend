...
export default function chatHandlers(io: Server, socket: Socket) {
  ...
  // When user joins
  socket.on('register_user', async ({ userId }: SocketData) => {
    if (!userId) return Promise.reject(new Error('register_user error: No userId provided.'));
    socket.join(userId);
  ...
  const transformedD2DChats = d2dChats.map(chat => {
    ...
    return {
      ...chat.toObject(),
      other,
      lastMessageTime: chat.lastMessage ? (chat.lastMessage as { createdAt?: Date }).createdAt : chat.updatedAt,
      unreadCount,
      type: 'd2d',
      dealers: undefined,
      name: other ? (other as { name?: string }).name : 'Unknown Dealer',
      profileImageUrl: other ? (other as { profileImageUrl?: string }).profileImageUrl : null,
    };
  });
  ...
  const transformedD2CChats = d2cChats.map(chat => {
    ...
    return {
      ...
      unreadCount,
      type: 'd2c',
      name: other ? (other as { name?: string }).name : 'Unknown Customer',
      profileImageUrl: other ? (other as { profileImageUrl?: string }).profileImageUrl : null,
    };
  });
  ...
  const transformedAIChats = aiChats.map(chat => ({
    ...
    unreadCount: 0,
    type: 'ai',
    name: (chat as { assistant?: { name?: string } }).assistant ? (chat as { assistant?: { name?: string } }).assistant?.name : 'AI Assistant',
  ...
  allChats.sort((a: unknown, b: unknown) => {
    const aTime = new Date((a as { lastMessageTime?: string | Date }).lastMessageTime || 0).getTime();
    const bTime = new Date((b as { lastMessageTime?: string | Date }).lastMessageTime || 0).getTime();
    return bTime - aTime;
  });
  ...
  socket.on('send_message', async ({
    ...
    try {
      ...
      if (chatType === 'd2d') {
        savedMessage = new DealerD2DMessage({ chatId: (chatDoc as { _id: string })._id, sender: senderId, content, type: effectiveMessageType });
        ...
        savedMessage = new DealerCustomerMessage({
          chatId: (chatDoc as { _id: string })._id,
          sender: senderId,
          senderRole,
          content,
      ...
      (chatDoc as { _id: string; lastMessage?: unknown; save: () => Promise<unknown> }).lastMessage = savedMessage?._id;
      await (chatDoc as { _id: string; lastMessage?: unknown; save: () => Promise<unknown> }).save();

      const participants = chatType === 'd2d'
        ? (chatDoc as unknown as { dealers: unknown[] }).dealers.map((id: unknown) => (id as { toString: () => string }).toString())
        : [(chatDoc as unknown as { dealer: { toString: () => string } }).dealer.toString(), (chatDoc as unknown as { customer: { toString: () => string } }).customer.toString()];

      const messageToEmit = { ...(savedMessage as { toObject: () => Record<string, unknown> }).toObject(), chatId: (chatDoc as { _id: { toString: () => string } })._id.toString(), tempId };
